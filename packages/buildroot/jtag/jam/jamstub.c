...
static inline int WritePin(int pin, int val)
{
	if (val)
		gpio2_bank->setdataout		= signals[pin];
	else
		gpio2_bank->cleardataout	= signals[pin];
	return 0;
}

static inline int ReadPin(int pin, int dir, int *val)
{
	if (dir == GPIO_IN)
		*val = (*( &(gpio3_bank->datain) ) & signals[pin]) ? 1 : 0;
	else
		*val = (*( &(gpio3_bank->dataout) ) & signals[pin]) ? 1 : 0;
	return 0;
}

/* Toggle TDI
 * if a specific line is 1, the corresponding bit should be 1 else it should be zero
 *  NOTE:   tck must be changed last 
 */
int WriteGPIO (int tms, int tdi, int tck)
{
	int rc;

	/* Write TMS */
	rc = WritePin(/*(cpld_num==1) ? VOLTAIRE_TMS_GPIO : VOLTAIRE_TMS_2_GPIO*/SIG_TMS, tms);
	if (rc < 0) {
		fprintf(stderr, "TMS write failed: %s\n", strerror(errno));
		return -1;
	}
small_delay();

	/* Write TDI */
	rc = WritePin(/*(cpld_num==1) ? VOLTAIRE_TDI_GPIO : VOLTAIRE_TDI_2_GPIO*/ SIG_TDI, tdi);
	if (rc < 0) {
		fprintf(stderr, "TDI write failed: %s\n", strerror(errno));
		return -1;
	}
small_delay();

	/* Write TCK */
	rc = WritePin(/*(cpld_num==1) ? VOLTAIRE_TCK_GPIO : VOLTAIRE_TCK_2_GPIO*/ SIG_TCK, tck);
	if (rc < 0) {
		fprintf(stderr, "TCK write failed: %s\n", strerror(errno));
		return -1;
	}
...
/* This function reads the value of the GPIO line connected to the TDO pin
 * and return its value (0 or 1) 
 */
int ReadGPIO_TDO(void)
{
	int rc;
	int val;
	
	rc = ReadPin(/*(cpld_num==1) ? VOLTAIRE_TDO_GPIO : VOLTAIRE_TDO_2_GPIO*/ SIG_TDO, GPIO_IN, &val); 
	if (rc < 0) {
		fprintf(stderr, "TDO read failed: %s\n", strerror(errno));
		return -1;
	}
...
int main(int argc, char **argv)
{
	...
	for (arg = 1; arg < argc; arg++)
	{
...
#endif
		{
			...
			case 'M':				/* set memory size */
				...
				break;
			case 'Z':
				cpld_num = atoi(&argv[arg][2]);
				if (cpld_num < 1 || cpld_num > 2) {
					fprintf(stderr, "cpld num should be 1 or 2\n");
					error == TRUE;
					help == TRUE;
				}
				break;
	...
	if (help || (filename == NULL))
	{
		...
		fprintf(stderr, "    -z<cpld num>: specidy cpld to upgrade\n");
	...
	{
		fprintf(stderr, "Error: no CPLD number specified\n");
		exit_status = 1;
	}
...
#define GPIO3_ADDRESS		0x481ae000

#define DEVMEM                  "/dev/mem"

int				memfd;
volatile gpio_bank_t		*gpio2_bank;
volatile gpio_bank_t		*gpio3_bank;


void hw_init(void)
{
	memfd = open(DEVMEM, O_RDWR);
	if (memfd >= 0)
	{
		gpio2_bank = (gpio_bank_t *)mmap(NULL, 0x1000,
						PROT_READ | PROT_WRITE,
						MAP_SHARED,
						memfd, GPIO2_ADDRESS);
		gpio3_bank = (gpio_bank_t *)mmap(NULL, 0x1000,
						PROT_READ | PROT_WRITE,
						MAP_SHARED,
						memfd, GPIO3_ADDRESS);
	}
